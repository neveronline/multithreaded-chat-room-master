...
int sort_by_err(const void *one, const void *two);
int sort_by_ip(const void *one, const void *two);
...
int main (int argc, char **argv){
  FILE *fp;
  int i, line_num;
  char *line[MAXLINE], buf[MAXBUF], name[MAXBUF], event[MAXBUF], from[MAXBUF], to[MAXBUF], result[MAXBUF], err[MAXBUF], keywords[MAXBUF];
  char file[MAXBUF];
  ...
  while(1){
    ...
    printf("\t- %s%s%s%s\n", CMD, "sbera: ", DEFAULT, "Sort by error code in ascending order (Available after \"fbe [CMD]\" then \"fbr [fail]\" or \"fbe [ERR]\")");
    printf("\t- %s%s%s%s\n", CMD, "sberd: ", DEFAULT, "Sort by error code in descending order (Available after \"fbe [CMD]\")");
    printf("\t- %s%s%s%s\n", CMD, "sbipa: ", DEFAULT, "Sort by ip address in ascending order (Available after \"fbe [LOGIN]\")");
    printf("\t- %s%s%s%s\n", CMD, "sbipd: ", DEFAULT, "Sort by ip address in dscending order (Available after \"fbe [LOGIN]\")");
    printf("\n");
    ...
    else if(!strcmp(buf, "sbera")){
      qsort (line, line_num, sizeof (char *), sort_by_err);
      for (i = 0; i < line_num; i++) {
        printf ("%s%d: %s%s", RESULT, i, line[i], DEFAULT);
      }
    }

    else if(!strcmp(buf, "sberd")){
      qsort (line, line_num, sizeof (char *), sort_by_err);
      for (i = line_num-1; i > 0; i--) {
        printf ("%s%d: %s%s", RESULT, i, line[i], DEFAULT);
      }
    ...
    else if(!strcmp(buf, "sbipa")){
      qsort (line, line_num, sizeof (char *), sort_by_ip);
      for (i = 0; i < line_num; i++) {
        printf ("%s%d: %s%s", RESULT, i, line[i], DEFAULT);
      }
    }

    else if(!strcmp(buf, "sbipd")){
      qsort (line, line_num, sizeof (char *), sort_by_ip);
      for (i = line_num-1; i > 0; i--) {
        printf ("%s%d: %s%s", RESULT, i, line[i], DEFAULT);
      }
...
int sort_by_err(const void *one, const void *two){
  ...
  char a_err[MAXBUF], b_err[MAXBUF], *ret;

  memset(a_err, '\0', MAXBUF);
  memset(b_err, '\0', MAXBUF);
...
int sort_by_ip(const void *one, const void *two){
  ...
  char a_ip[MAXBUF], b_ip[MAXBUF];
  int i, j, length;
  memset(a_ip, '\0', MAXBUF);
  memset(b_ip, '\0', MAXBUF);
  ...
  i = i-length;
  strncpy(b_ip, &b[i], length);

  return strcmp(a_ip, b_ip);
